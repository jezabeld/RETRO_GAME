/**
 * @file AccelJoystickDrv.h
 * @brief Complete MPU6050 accelerometer driver with I2C DMA continuous reading
 *
 * Full-featured driver that initializes, calibrates, and continuously reads
 * the MPU6050 accelerometer using timer-triggered I2C DMA transfers.
 * No dedicated RTOS task required.
 *
 * @date Nov 28 2025
 * @author jez
 */

#ifndef ACCEL_JOYSTICK_DRV_H_
#define ACCEL_JOYSTICK_DRV_H_
...
/**
 * @brief Accelerometer tilt data structure
 */
typedef struct {
    int16_t tiltX; ///< X-axis tilt (calibrated, filtered)
    int16_t tiltY; ///< Y-axis tilt (calibrated, filtered)
} accelTilt_t;
...
/**
 * @brief Get latest filtered tilt values
 *
 * Thread-safe, non-blocking read of last processed accelerometer data.
 * Values are calibrated (centered at 0) and filtered with EMA + deadband.
 *
 * @param tilt Pointer to structure to receive tilt data
 *
 * @note Values range approximately ±16384 for ±1G tilt
 * @note Deadband applied: ±100 counts = ±0.006G
 */
void accelDrvGetTilt(accelTilt_t *tilt);

#endif /* ACCEL_JOYSTICK_DRV_H_ */
