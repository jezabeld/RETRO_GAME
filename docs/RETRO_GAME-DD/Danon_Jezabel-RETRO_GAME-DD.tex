...
\textbf{Lógica del sistema} & \texttt{BootMng} & Gestor de arranque & Inicializa drivers, crea tareas FreeRTOS, transfiere a menú principal \\
\cline{2-4}
...
\subsubsection{\texttt{BootMng} — Gestor de arranque}
...
\begin{itemize}
  ...
  \item Responsable de invocar a \texttt{UpdateEngine} y manda eventos a \texttt{RenderEngine}.
  \item Emite eventos como \texttt{SE\_PAUSE\_REQ}.
  \item Debe poder congelar el juego sin perder el estado (ej. en pausa o guardado).
  \item Genera gameModel que encapsule subt-structs: estado de la navegación y del escenario renderizado
  \begin{itemize}
    ...
    \item 
  \end{itemize}
  \item Genera gameSignal[]: vector / cola de “hechos relevantes” (que requieran sonidos o vibracion)
  % \begin{itemize}
...
Máquina de estado del sistema con subestados de juego y pausa.
...
En la figura \ref{fig:diagBootMng} puede observarse la secuencia interna del Boot Manager. En la figura \ref{fig:diagInitMenu}

\begin{figure}[h]
\centering 
\includegraphics[width=.85\textwidth]{../Figuras/zoom_bootmng_flow.pdf}
\caption{Zoom secuencia interna del BootMng.}
\label{fig:diagBootMng}
\end{figure}
...
\begin{figure}[h!]
\centering 
\includegraphics[width=.85\textwidth]{../Figuras/init_menu_flow.pdf}
\caption{Flow desde boot hasta menú inicio.}
\label{fig:diagInitMenu}
\end{figure}

\newpage
\subsection{Diagrama de secuencia 2 - Comenzar nuevo juego}

\begin{figure}[h!]
\centering 
\includegraphics[width=.85\textwidth]{../Figuras/start_new_game_flow.pdf}
\caption{Flow desde menú inicio hasta comenzar nuevo juego.}
\label{fig:diagStartNew}
\end{figure}
