...
/**
 * @brief Apply EMA filter, deadband, and normalization to raw accelerometer data
 *
 * Processes raw accelerometer data through filtering pipeline and normalizes to ±1024 range.
 */
static void processAccelData(const uint8_t *buf) {
    ...
    // Normalize to -1024..+1023 range (±16384 → ±1024, 2048 values)
    int16_t normX = ((int32_t)outX * 1024) / 16384;
    int16_t normY = ((int32_t)outY * 1024) / 16384;
    ...
    published.tiltX = normX;
    published.tiltY = normY;
}
