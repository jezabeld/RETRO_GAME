...
//    tftInit(&myTft, &hspi1, TFT_CS_GPIO_Port, TFT_CS_Pin, TFT_DC_GPIO_Port, TFT_DC_Pin, TFT_RS_GPIO_Port, TFT_RS_Pin);
    ...
    /*Finally register the driver y setea el flag de inicializacion */
    disp_drv_ready = lv_disp_drv_register(&disp_drv);
}
...
static void disp_flush(lv_disp_drv_t* drv, const lv_area_t* area, lv_color_t* color_p) {
    //	 LV_LOG_INFO("FLUSH %d,%d %d,%d", area->x1, area->y1, area->x2,
    // area->y2);
