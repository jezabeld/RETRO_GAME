...
/* ===== API ===== */
void audioPlayerInit(uint32_t fs_hz)
{
    s_fs = fs_hz;
    audioInit(fs_hz);
    audioSetFill12Fn(fill12);
    audioStart();
}
void audioPlayerStartMusic(const note_t* song, uint32_t len, uint8_t loop)
{
    s_song = song; s_len = len; s_idx = 0; s_loop = loop ? 1:0; s_music = 1;
    s_phase = 0.0f; s_fade = 0; s_left = 0;
    next_music();
}
void audioPlayerStopAll(void)
{
    s_music = 0; s_sfx_on = 0; s_song = 0; s_len = s_idx = 0;
    set_note(0, WF_SINE, 0, 1);
}
void audioPlayerPlaySfx(note_t sfx)
{
    s_sfx = sfx; s_sfx_on = 1;
    set_note(sfx.freq_hz, sfx.wave, sfx.amp12, sfx.dur_ms);
}
void audioPlayerSetVolume(float g)
{
    if (g < 0.0f) g = 0.0f;
    if (g > 1.0f) g = 1.0f;
    s_gain = g;
