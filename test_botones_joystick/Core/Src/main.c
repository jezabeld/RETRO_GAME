...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	switch (GPIO_Pin) {
	case BTN_LEFT_Pin:   name = "LEFT";  break;
	case BTN_DOWN_Pin:   name = "DOWN";  break;
	case BTN_RIGHT_Pin:  name = "RIGHT"; break;
	case BTN_UP_Pin:     name = "UP";    break;
...
/*-----------------------------------------------------------------------
 * Lee los 2 canales configurados en el ADC secuencialmente
 *---------------------------------------------------------------------*/
static void read_joystick_once(void)
{
    /* inicia conversión “regular” de la secuencia de 2 ranks */
    HAL_ADC_Start(&hadc1);

    /* Rank 1 → PA1 */
    HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY);
    joy_raw[0] = HAL_ADC_GetValue(&hadc1);

    /* Rank 2 → PA4 */
    HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY);
    joy_raw[1] = HAL_ADC_GetValue(&hadc1);

    HAL_ADC_Stop(&hadc1);   /* no es imprescindible */
}
...
int main(void)
{
  ...
  uartSendString("BTN + Joystick test\r\n");

  HAL_ADC_Start_DMA(&hadc1, joy_raw, 2);
